...
class sqlQuiz extends HTMLElement {
  ...
  connectedCallback() {
    ...
    submitButton.value = 'Comprobar respuestas';
    inputArea.appendChild(submitButton);

    var hintButton = document.createElement('input');
    hintButton.name = "hint";
    hintButton.type = "button";
    hintButton.value = "Mostrar explicaciones";
    ...
    form['onsubmit'] = (e) => {
      ...
      var res = mistakes >= 2 ? mistakes + " errores" :
          mistakes == 1 ? mistakes + " error" : "¡Todo correcto!"
...
class sqlExercise extends HTMLElement {
  ...
  connectedCallback() {
    ...
    var runButton = `<input class="sql-exercise-submit" type="submit" value="Ejecutar &#x21e9;" disabled>`;
      ...
      var handleSubmit = (submission_data) => {
        ...
        if (solution) {
          ...
          query(solution, (solution_data) => {
            ...
            var verdict = arraysEqual(submission_u, solution_u) ? "Correcto" : "Incorrecto";
        ...
        if (submission_data.length > 0) {
          ...
          result_div.insertAdjacentHTML("beforeend", `No se devolvieron datos`);
        }
      }
    ...
    if (solution) {
      ...
      solutionButton.value = 'Mostrar solución';
